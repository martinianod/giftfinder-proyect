...
class ProviderCapabilities(BaseModel):
    ...
    supportsImages: bool = Field(
        default=True, description="Provider returns product images"
    )
    supportsPriceFilter: bool = Field(
        default=False, description="Provider can filter by price range"
    )
    supportsLocation: bool = Field(
        default=False, description="Provider supports location-based filtering"
    )
    supportsStock: bool = Field(
        default=False, description="Provider returns stock availability"
    )
    supportsDeepLink: bool = Field(
        default=True, description="Provider returns direct product URLs"
    )
    supportsCategories: bool = Field(
        default=False, description="Provider supports category filtering"
    )
    supportsRatings: bool = Field(
        default=False, description="Provider returns product ratings"
    )
    supportsShipping: bool = Field(
        default=False, description="Provider returns shipping information"
    )
...
class RecipientProfile(BaseModel):
    ...
    type: str = Field(
        default="unknown",
        description="Recipient type (friend, family, colleague, etc.)",
    )
    ...
    interests: List[str] = Field(
        default_factory=list, description="Recipient interests"
    )
    relationship: Optional[str] = Field(
        default=None, description="Relationship to recipient"
    )
...
class ProductQuery(BaseModel):
    ...
    priceMin: Optional[float] = Field(
        default=None, ge=0, description="Minimum price filter"
    )
    priceMax: Optional[float] = Field(
        default=None, ge=0, description="Maximum price filter"
    )
    currency: Optional[str] = Field(
        default=None, description="Currency code (ISO 4217)"
    )
    locale: Optional[str] = Field(
        default=None, description="Locale (e.g., 'es-AR', 'en-US')"
    )
    location: Optional[LocationFilter] = Field(
        default=None, description="Location-based filtering"
    )
    ...
    limit: int = Field(
        default=10, ge=1, le=30, description="Max results to return (clamped to 30)"
    )
    excludeVendors: List[str] = Field(
        default_factory=list, description="Vendor IDs to exclude"
    )
    includeVendors: List[str] = Field(
        default_factory=list, description="Vendor IDs to include (whitelist)"
    )
...
class ProductAvailability(BaseModel):
    ...
    inStock: Optional[bool] = Field(
        default=None, description="Whether product is in stock"
    )
    stockCount: Optional[int] = Field(
        default=None, ge=0, description="Available stock count"
    )
    leadTimeDays: Optional[int] = Field(
        default=None, ge=0, description="Lead time in days for delivery"
    )
...
class ProductRating(BaseModel):
    ...
    value: Optional[float] = Field(
        default=None, ge=0, le=5, description="Rating value (0-5)"
    )
...
class Product(BaseModel):
    ...
    currency: str = Field(
        default="ARS", description="Currency code (ISO 4217 if possible)"
    )
    ...
    raw: Optional[Dict[str, Any]] = Field(
        default=None, description="Raw provider data (for debugging)"
    )
...
class ProviderMetadata(BaseModel):
    ...
    fetchedAt: datetime = Field(
        default_factory=datetime.now, description="Timestamp of fetch"
    )
    latencyMs: int = Field(description="Response time in milliseconds")
    warnings: List[str] = Field(
        default_factory=list, description="Any warnings or errors"
    )
...
class ProviderContext(BaseModel):
    ...
    trace: Optional[Dict[str, str]] = Field(
        default=None, description="Tracing information (spanId, etc.)"
    )
